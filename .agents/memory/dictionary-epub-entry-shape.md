...
**How to apply:** Parse a colon entry together with the next non-empty non-example line as its translation. Store the English expression after the colon as the term, stripping parenthetical usage slots from its search term while retaining the complete phrase as the sense definition; ignore bilingual example lines during import.