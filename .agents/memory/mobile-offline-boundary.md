...
**How to apply:** Keep Ollama endpoint/model settings and generation flows in the computer web app. Browser-side private generation must call the user's local Ollama directly; never route private source text through the remote API. Treat synchronization as an occasional download boundary, show the last sync status on mobile, and make reading, cards, semantic maps, and review work from SQLite/offline data.